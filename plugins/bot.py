...
buttons = [
    [
        ...
        Button.url(get_string("bot_4"), "t.me/AppleBotz"),
    ]
]
...
async def _(event):
    ...
    file = f"apple{sys.argv[-1]}.log" if len(sys.argv) > 1 else "apple.log"
    if opt == "heroku":
        ...
        file = await Carbon(
            file_name="apple-logs",
            code=code,
            backgroundColor=choice(ATRA_COL),
        )
        await event.reply("**Apple Logs.**", file=file)
    elif opt == "open":
        file = open("apple.log", "r").read()[-4000:]
...
async def _(e):
    ...
    if m:
        ...
        await xx.edit(
            f'<code>Your BOT is </code><strong>up-to-date</strong><code> with </code><strong><a href="https://github.com/AppleBotz/Matrix/tree/{branch}">[{branch}]</a></strong>',
            parse_mode="html",
            link_preview=False,
        )
